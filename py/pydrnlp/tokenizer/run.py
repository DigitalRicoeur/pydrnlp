...
A simple interface to `pydrnlp.tokenizer.tokenize` based on standard IO.

This module does nothing unless `__name__ == "__main__"`.

When run, it immediately writes the JSON result of
`pydrnlp.tokenizer.tokenize.revision()` to standard out.
It then enters an JSON IO loop (see `pydrnlp.jsonio.mapJsonLines()`),
processing the values with `pydrnlp.tokenizer.tokenize.tokenize`.
